;; 
;; problem: 3-mods-use-four-slots
;; 
;; This problem requires all four slots of the four-slot backplane to 
;; be used.  No objects should be added to the order.  There are two 
;; s-p-c (standard pinning "class?") modules that should fit into the 
;; 4-slot backplane.  The deuna-aa has 2 boards, and the lp11 has 1 
;; board each.
;;

(load-goal '(configuration-complete))

(load-start-state '((order 1 deuna-aa mod)
		    (order 2 lp11 mod)
		    (order 1 standard box)
		    (order 1 four-slot bp)
		    (order 1 jumper cable)
		    (order 1 unibus system)

		    (can-add-items-to-order no)
