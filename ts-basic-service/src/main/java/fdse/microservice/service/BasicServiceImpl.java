...
public class BasicServiceImpl implements BasicService{
    ...
    public ResultForTravel queryForTravel(QueryForTravel info){
        ...
        Boolean startingPlaceExist = restTemplate.postForObject(
                "https://ts-station-service:12345/station/exist", new QueryStation(info.getStartingPlace()), Boolean.class);
        Boolean endPlaceExist = restTemplate.postForObject(
                "https://ts-station-service:12345/station/exist", new QueryStation(info.getEndPlace()),  Boolean.class);
        if(!startingPlaceExist || !endPlaceExist){
            result.setStatus(false);
        }

        String startingPlaceId = restTemplate.postForObject(
                "https://ts-station-service:12345/station/queryForId", new QueryStation(info.getStartingPlace()), String.class);
        String endPlaceId = restTemplate.postForObject(
                "https://ts-station-service:12345/station/queryForId", new QueryStation(info.getEndPlace()),  String.class);


        //配置
        //查询车票配比，以车站ABC为例，A是始发站，B是途径的车站，C是终点站，分配AC 50%，如果总票数100，那么AC有50张票，AB和BC也各有
        //50张票，因为AB和AC拼起来正好是一张AC。
        String proportion = restTemplate.postForObject("https://ts-config-service:15679/config/query",
        ...
        TrainType trainType = restTemplate.postForObject(
                "https://ts-train-service:14567/train/retrieve", new QueryTrainType(info.getTrip().getTrainTypeId()), TrainType.class
        );
        if(trainType == null){
        ...
        String priceForEconomyClass = restTemplate.postForObject(
                "https://ts-price-service:16579/price/query",queryPriceInfo , String.class
        );
