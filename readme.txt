#########################################################
故障重现：
    两段本应有先后顺序的异步任务，异步掌握不好导致顺序错乱
    错误位置：退票流程 - 退订普通车票
    正常流程：1.将订单状态改为-退款中，然后退款
             2.将订单状态改为-已退款
    错误流程：1.将订单状态改为-已退款
             2.将订单状态改为-退款中，然后退款
    结果：正常流程最后的订单状态为【已取消】
         错误流程的最后的订单状态为【退款中】，页面显示【other】
    触发流程：在页面点击退票
             有一半的概率是正常流程
             一半的概率是错误流程
             正常流程控制台会输出 - [Cancel Order Service] 正常流程，退票应该正常
             错误流程控制台会输出 - [Cancel Order Service] 延迟流程，退票将会错误
#########################################################
